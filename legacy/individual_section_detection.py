...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 13 19:06:45 2018
...
def findHalfCircle(radius, delta):
    ...
    for i in range(0,math.ceil(maxR)+1):
        ...
        while (distPix<maxR):
            if (distPix>minR):
                halfCircle = np.append(halfCircle,[[i,j]], axis = 0)#(check.shape[0]+1,2))
            j+=1
            distPix = dist(0,0,i,j)
...
def cordLine(cor1, cor2):
    ...
    #output:    - cordinates spanning the line between the two input coordinates
...
def overlapIndividual(viaEdges, viaFz):
    #input:     - one boolean matrix and one integer matrix of the same size
    #output:    - integer matrix indicating, indicating 0 for non-section, and 1,..,n for each section n
     
    #detect background
    u, indices = np.unique(viaFz, return_inverse=True)
    background = u[np.argmax(np.bincount(indices))]
    
    sections = np.zeros((viaEdges.shape[0], viaEdges.shape[1]))
    sectionNumbers = np.zeros((1,2))
    sectionNumbers[0,1] = background
    for i in range(0, viaEdges.shape[0]):
        for j in range(0, viaEdges.shape[1]):
            if viaEdges[i,j] == True and viaFz[i,j] is not background:
                oldSection, sectionNumber = getSectionNumber(viaFz[i,j], sectionNumbers)
                sections[i,j] = sectionNumber
                if not oldSection:
                    sectionNumbers = np.append(sectionNumbers,[[sectionNumber, viaFz[i,j]]], axis = 0)
    return sections

                
def getSectionNumber(test, sectionNumbers):
    oldSection = False
    sectionNumber = sectionNumbers[:,0].max()+1
    
    for i in range(sectionNumbers.shape[0]):
        if sectionNumbers[i,1] == test:
            sectionNumber = sectionNumbers[i,0]
            oldSection = True
    
    return oldSection, sectionNumber


def downsizeTo(im, size):
    #input:     - image which need to be downsized to size size
...
def findCircle(edges, ratio):
    #input:     - boolean matrix that is True for pixels containing an edge
...
def findBorderPerLine(c): #INPUT IS SORTED!!
